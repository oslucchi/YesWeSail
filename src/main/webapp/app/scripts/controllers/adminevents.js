        ...
        $http.post(URLs.ddns + 'rest/events/search/all', {}).then(function (res) {
            ...
            $('thead th.id').data('sortBy', function(th, td, tablesort) {
               ...
               $scope.gridOptions = {
                    ...
                    columnDefs: [
            { enableFiltering: false, minWidth: 220, width: 220, name: 'Actions', field:"", cellTemplate: '<div class="ui-grid-cell-contents"><i ng-click="grid.appScope.activate(row.entity)" class="large green checkmark link icon"></i> <i ng-really-message="Are you sure?" ng-really-click="remove(row.entity)" class="large red trash outline link icon"></i> <i ng-click="grid.appScope.deactivate(row.entity)" class="large red ban link icon"></i> <i ng-click="grid.appScope.showClonePopup(row.entity)" class="large grey copy link icon"></i> <i ui-sref="editEvent({eventId: row.entity.idEvents})" class="large grey edit link icon"></i> <i ng-click="grid.appScope.showAddPassengerPopup(row.entity.idEvents)" class="large grey add user link icon"></i> <i ng-click="grid.appScope.showRemovePassengerPopup(row.entity.idEvents)" class="large grey remove user link icon"></i></div>'},
          { minWidth: 65, width: 65, name:'Status', field: 'status', cellTemplate: '<div class="ui-grid-cell-contents"> <div ng-switch="row.entity.status"><div ng-switch-when="A"><i class="large green checkmark icon"></i></div><div ng-switch-when="I"><i class="large red remove icon"></i></div><div ng-switch-when="P"><i class="large yellow wait icon"></i></div><div ng-switch-default> </div></div></div>' },
          {minWidth: 65, width: 65, name:'Hot', field: 'hotEvent', cellTemplate: '<div class="ui-grid-cell-contents"><div class="ui fitted center  checkbox"> <input type="checkbox" ng-model="row.entity.hotEvent" ng-change="grid.appScope.updateHotEvent(row.entity, row.entity.hotEvent)" ng-checked="row.entity.hotEvent">   <label></label> </div></div>' },
          {minWidth: 65, width: 65, name:'Early', field: 'earlyBooking', cellTemplate: '<div class="ui-grid-cell-contents"><div class="ui fitted center  checkbox"> <input type="checkbox" ng-model="row.entity.earlyBooking" ng-change="grid.appScope.updateEarlyBooking(row.entity, row.entity.earlyBooking)" ng-checked="row.entity.earlyBooking">   <label></label> </div></div>' },
          {minWidth: 65, width: 65, name:'Last Min.', field: 'lastMinute', cellTemplate: '<div class="ui-grid-cell-contents"><div class="ui fitted center  checkbox"> <input type="checkbox" ng-model="row.entity.lastMinute" ng-change="grid.appScope.updateLastMinute(row.entity, row.entity.lastMinute)" ng-checked="row.entity.lastMinute">   <label></label> </div></div>' },
          {minWidth: 65, width: 65, name:'ID', field: 'idEvents' },
          { name:'Titolo', field: 'title' },
            {
                name:'ID Armatore', field: 'shipOwnerId'},
            {
            ...
            name:'Armatore', field: 'shipOwnerId', cellTemplate: '<div class="ui-grid-cell-contents">{{grid.appScope.shipOwners[row.entity.shipOwnerId].name}} {{grid.appScope.shipOwners[row.entity.shipOwnerId].surname}}</div>' },    
            {
                enableFiltering: false,
                sort: {
                direction: 'desc',
                priority: 1
            }, name:'Data Inizio', field: 'dateStart',  cellTemplate: '<div class="ui-grid-cell-contents">{{row.entity.dateStart | date: "yyyy/MM/dd"}}</div>'  },
          {enableFiltering: false, name:'Data Fine', field: 'dateEnd', cellTemplate: '<div class="ui-grid-cell-contents">{{row.entity.dateEnd | date: "yyyy/MM/dd"}}</div>'  },
          {minWidth: 103, width: 103, name:'Creato da', field: 'createdBy' },
          {enableFiltering: false, name:'Creato il', field: 'createdOn',  cellTemplate: '<div class="ui-grid-cell-contents">{{row.entity.createdOn | date: "yyyy/MM/dd"}}</div>'  },
          { name:'Meta', field: 'location' }
        ],
